...
%%%
vidFlag = 0; %whether or not to load data from the video clip
...
xlabel('$x$ (centimeters)','FontSize',labelSize)
ylabel('$y$ (centimeters)','FontSize',labelSize)
...
for ii = loc_idx
    ...
    for jj = stop_steps
        step = jj-1:jj;
        plot(x_line(ii, step), y_line(ii, step), "color", colors(7,:), 'LineWidth', 1.5 )
    end
    for jj = crawl_steps
        step = jj-1:jj;
        plot(x_line(ii, step), y_line(ii, step), "color", yellow, 'LineWidth', 1.5 )
    end
    for jj = hop_steps
        step = jj-1:jj;
        plot(x_line(ii, step), y_line(ii, step), "color", blueishgreen, 'LineWidth', 1.5 )
    end
end
...
myQuiv(idx_stop(:,T_end), colors(7,:), x_line(:,T_end), y_line(:,T_end), theta_line(:,T_end), scalefactor)
myQuiv(idx_crawl(:,T_end), yellow, x_line(:,T_end), y_line(:,T_end), theta_line(:,T_end), scalefactor)
myQuiv(idx_hop(:,T_end), blueishgreen, x_line(:,T_end), y_line(:,T_end), theta_line(:,T_end), scalefactor)
...
%% Associated Functions %%
    ...
    function myQuiv(idx,color, xposn, yposn, theta, scalefactor)
